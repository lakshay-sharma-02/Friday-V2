...
The original V8 plan (gates G1–G6 + 13 composite tasks) is **done**;since then the core was hardened, a 515-test unit suite was added, a live
...
|----------|----------------|
| `TESTS_PROOF.md` | 515-test dependency-free unit suite, all side-effect boundaries mocked (regenerated by `gates/test_suite.py`) |
| `CAPABILITIES.md` | Live capability inventory GENERATED from the registry (57 primitives / 17 checks / 10 triggers / 10 gate-registered) — `gates/generate_capabilities.py`, idempotent |
...
`tests/` — 515 tests, stdlib `unittest`, zero dependencies, ~18 s. (8 of them
...
0. **Capability-gap approval gate — RESOLVED (automated + human)**: the
   ...
   **10 gate-registered primitives** in total.
...
```bash
./.venv/bin/python gates/test_suite.py            # 515 tests -> TESTS_PROOF.md
