...
The original V8 plan (gates G1–G6 + 13 composite tasks) is **done**;since then the core was hardened, a 535-test unit suite was added, a live
...
|----------|----------------|
| `TESTS_PROOF.md` | 535-test dependency-free unit suite, all side-effect boundaries mocked (regenerated by `gates/test_suite.py`) |
| `CAPABILITIES.md` | Live capability inventory GENERATED from the registry (58 primitives / 17 checks / 10 triggers / 11 gate-registered) — `gates/generate_capabilities.py`, idempotent |
...
`tests/` — 535 tests, stdlib `unittest`, zero dependencies, ~14 s. (8 of them
new in tests/test_calendar.py covering the 2026-08-14 auth fix.) Covers
...
0. **Capability-gap approval gate — RESOLVED (automated + human)**: the
   ...
   **11 gate-registered primitives** in total (2026-08-15 added
...
```bash
./.venv/bin/python gates/test_suite.py            # 535 tests -> TESTS_PROOF.md
