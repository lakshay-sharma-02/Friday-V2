...
The original V8 plan (gates G1–G6 + 13 composite tasks) is **done**;since then the core was hardened, a 439-test unit suite was added, a live
...
L1  Primitives   (window/media/browser/dev/files/git/digestcheck/gmail/notify/whatsapp/telegram/discord)
                 (git.log + files.find_recent_doc + files.read_text + dev.digest
...
|----------|----------------|
| `TESTS_PROOF.md` | 439-test dependency-free unit suite, all side-effect boundaries mocked (regenerated by `gates/test_suite.py`) |
| `CAPABILITIES.md` | Live capability inventory GENERATED from the registry (49 primitives / 17 checks / 6 triggers / 2 gate-registered) — `gates/generate_capabilities.py`, idempotent |
...
`tests/` — 463 tests, stdlib `unittest`, zero dependencies, ~20 s. Covers
...
gate6_passed, timestamp, proof`); failures are never deleted. **22 distinct
passing ids**: the 13 composite tasks, the `retry-stress` gate, and 8
...
`watch:weekly-cross-project-digest`).
...
- **Capability-gap drafts are unvalidated LLM output**: triage drafts are
  ...
  yet are wrong. They are rejected at HUMAN REVIEW; the gate never
  registers a draft without the human signature, so a bad draft changes
...
- **The LLM layer is at the mercy of the local model router
  (2026-08-13)**: the opus alias routes through the user's local proxy to
...
- **Watch loop IS deployed — one honest behavior left**: the daemon runs
  ...
  logout (`loginctl enable-linger` is the documented opt-in). And  the committed `ambient-gap-probe-calendar` trigger (11:00 daily) still
  generates ambient capability-gap volume DELIBERATELY: silent
  (notify:false), an allowlisted deterministic probe whose single step is a
  genuinely UNBUILT primitive (`calendar.list_upcoming`)
  — refused by the allowlist BEFORE any execution (nothing ever runs) and
  ...
  The `ambient-gap-probe-email-send` trigger was **RETIRED (2026-08-11)**
  after `gmail.send_document` was approved + registered (a daily refusal
  of a solved primitive is noise), and `morning-gmail-summary`'s allowlist
  was tightened from `"gmail.*"` to the three read-only primitives so the
  LLM-planned morning trigger can never reach the send primitive. The
  heartbeat's `capability_gaps` count reflects the one remaining probe. The `sunday-digest-reminder` trigger (10:05 Sundays, notify-only,
...
0. **Capability-gap approval gate — RESOLVED (automated + human)**: the
   ...
   **`calendar.list_upcoming` (2026-08-13 — the loop's SECOND complete
   ...
   repair loop and every automated gate unchanged, then registered)**.
...
6. **MCP servers**: expose Friday's primitives as an MCP tool server so
   other agents can drive the desktop through verified primitives (the
   L0/L2 machinery is already the right boundary).

## How to verify anything in this document

```bash
./.venv/bin/python gates/test_suite.py            # 463 tests -> TESTS_PROOF.md
