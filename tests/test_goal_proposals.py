...
class TestMine(EnvTestCase):
    ...
    def _rec(self, goal: str, *, days_ago: int = 1, passed: bool = False,
             task_id: str = "proof-run", refused: bool = False) -> dict:
        rec = {
            "task_id": task_id, "goal": goal, "gate6_passed": passed,
            "timestamp": _ts(days_ago),
            "proof": json.dumps({"status": "REFUSED"}) if refused else json.dumps({"status": "ABORT"}),
        }
        return rec

    def test_clusters_recurring_failed_goals(self):
        self._seed([
            self._rec(GOAL_A), self._rec(GOAL_A, days_ago=2), self._rec(GOAL_A, days_ago=3),
    ...
    def test_sorted_by_occurrences(self):
        self._seed([
            self._rec(GOAL_B), self._rec(GOAL_B, days_ago=2), self._rec(GOAL_B, days_ago=3),
            self._rec(GOAL_A), self._rec(GOAL_A, days_ago=2),
        ])
        clusters = mine()
    ...
    def test_watch_l0_evidence_attached(self):
        Path(os.environ["FRIDAY_LOG_FILE"]).write_text(
            json.dumps({"layer": "WATCH", "primitive": "trigger", "result": "FAILED",
                        "args": {"id": "x", "goal": GOAL_B}, "extra": {"status": "ABORT"}}) + "\n",
            encoding="utf-8",
        )
        self._seed([self._rec(GOAL_B), self._rec(GOAL_B, days_ago=2)])
        c = mine()[0]
...
class TestDraft(EnvTestCase):
    ...
    def _cluster(self) -> dict:
        return {"goal": GOAL_B, "occurrences": 2, "task_ids": ["a", "b"],
                "timestamps": [_ts(1), _ts(2)], "last_failed_at": _ts(1),
                "l0_evidence": []}
    ...
    def test_verbatim_goal_survives_llm_path(self):
        with mock.patch("friday.l1.dev._run_claude",
                        return_value={"result": json.dumps({
                            "id": "pause-close", "schedule": {"type": "time", "at": "18:30", "days": ["mon"]},
                            "allow": ["media.*", "window.*"], "rationale_note": "keep the desktop clean nightly"})}):
    ...
    def test_llm_non_time_schedule_falls_back(self):
        with mock.patch("friday.l1.dev._run_claude",
                        return_value={"result": json.dumps({
                            "id": "x", "schedule": {"type": "file", "directory": "/tmp"},
                            "allow": []})}):
            t = _draft_trigger(self._cluster(), use_llm=True)
        self.assertEqual(t["schedule"]["type"], "time")
    ...
    def test_unique_id_avoids_existing_trigger_ids(self):
        config = Path(os.environ["FRIDAY_WATCHER_CONFIG"])
        config.write_text(json.dumps({"triggers": [{
            "id": "pause-whatever", "goal": "unrelated thing",
            "schedule": {"type": "time", "at": "09:00"},
        }]}), encoding="utf-8")
...
class TestPropose(EnvTestCase):
    def setUp(self):
        ...
        with open(path, "a", encoding="utf-8") as fh:
            for ago in (1, 2):
                fh.write(json.dumps({
                    "task_id": f"run-{ago}", "goal": GOAL_B, "gate6_passed": False,
                    "timestamp": _ts(ago),
    ...
    def test_llm_draft_note_stays_out_of_trigger_json(self):
        ...
        self.assertIn("keep the desktop clean nightly", (d / "rationale.md").read_text(encoding="utf-8"))
    ...
    def test_existing_proposal_dir_covered(self):
        ...
        with open(path, "a", encoding="utf-8") as fh:
            fh.write(json.dumps({
                "task_id": "z", "goal": "send a daily standup note to my whatsapp",
                "gate6_passed": False, "timestamp": _ts(1),
                "proof": json.dumps({"status": "ABORT"}),
            }) + "\n")
            fh.write(json.dumps({
                "task_id": "z2", "goal": "send a daily standup note to my whatsapp",
                "gate6_passed": False, "timestamp": _ts(2),
                "proof": json.dumps({"status": "ABORT"}),
            }) + "\n")
...
class TestHelpers(EnvTestCase):
    ...
    def test_goal_covered_substring_and_token_overlap(self):
        triggers = [{"goal": "find the most recent unread email from $facts.gmail_sender and summarize it in at most 5 plain sentences"}]
