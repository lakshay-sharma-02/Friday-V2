...
protected windows, dangerous-dev gate); a **439-test unit suite** covers
...
tests/               dependency-free unittest suite (463 tests, all mocked)
  l2/
...
| `TESTS_PROOF.md` | The dependency-free unit suite, raw output (regenerated by `gates/test_suite.py`; 463 tests) |
| `CAPABILITIES.md` | Live capability inventory GENERATED from the contract registry — 49 primitives, 17 checks, 6 triggers, gate-registered primitives (regenerate: `./.venv/bin/python gates/generate_capabilities.py`, idempotent) |
...
The `ambient-gap-probe-calendar` trigger (11:00 daily, enabled) is the
remaining **deliberate ambient volume source for the self-improvement
loop**: silent (`notify: false`), allowlisted
(`allow: ["notify.notify_send"]`), a deterministic probe whose single step
is the genuinely UNBUILT primitive `calendar.list_upcoming`. The
allowlist refuses the step BEFORE any execution — nothing ever runs — and
REFUSED is terminal-for-the-day, so it records **exactly one real
capability-gap record per day** for triage to group and LLM-draft. The
`ambient-gap-probe-email-send` trigger was **RETIRED (2026-08-11)** after
its primitive (`gmail.send_document`) was approved and registered — a
daily refusal of a solved primitive is noise. And because registering a
send primitive under a `gmail.*` pattern would have silently armed the
LLM-planned morning trigger with send capability, that trigger's
allowlist was tightened to the three read-only primitives
...
values — and refusal_reason). Two committed `ambient-gap-probe-*`
triggers now generate this ambient volume deliberately — see the watch-loop
section — so the loop's volume and draft quality are tested under real
continuous operation, not just by proof runs. `friday/gap_triage.py` groups unprocessed
